...
public class ListOrderTest extends BaseTest{
	...
	private String url = "http://rsm.58.com/demand/order/listData";
	...
	@Test(dataProviderClass=ListOrderProvider.class,dataProvider="listOrderProvider")
	public void listOrderTest(String msg,String pageSize) throws Exception{
		...
		Assert.assertEquals(message.toString(), "SUCCESS", "获取订单列表接口返回状态码错误！");
		
		//数据库返回的数据数目
		String condition = "SELECT count(*) from t_rs_demand where serve_model = 1 AND staff_id = '871980545922351104'";
		...
		if(page==1){
			Assert.assertEquals(object.getJSONArray("data").size(), except, "获取订单个数与数据库中不一致");
		}else{
			for(int i=1;i<=page;i++){
				...
				if(i<page){
					Assert.assertEquals(String.valueOf(actual), pageSize, "第"+i+"页获取订单数目与数据库不一致");
				}else{
					int end_num = except - pagesize *(i-1);
					System.out.println(end_num);
					Assert.assertEquals(String.valueOf(actual), String.valueOf(end_num), "第"+i+"页获取订单数目与数据库不一致");
				}
			}
