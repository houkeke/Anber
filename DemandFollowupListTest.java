...
import com.bj58.qa.rs.inteface.dataprovider.DemandFollowupListProvider;
 ...
 */
public class DemandFollowupListTest extends BaseTest{

	private String url = "http://rsm.58.com/demand/followup/listData";
	...
	@Test(dataProviderClass=DemandFollowupListProvider.class,dataProvider="demandFollowupListProvider")
	public void demandFollowupTest(String msg,String pageSize) throws Exception{
		...
		Assert.assertEquals(message.toString(), "SUCCESS", "待跟进列表接口返回状态码错误");
		
		//数据库返回的数据数目
		//待审核列表中展示的需求为未绑定客服的需求和绑定的客服为自己的需求
		String condition = "select count(*) from t_rs_demand where state=2 AND staff_id = '871980545922351104'";
		...
		if(page==1){
			Assert.assertEquals(response.getJSONArray("data").size(), except_size, "获取待跟进需求个数与数据库中不一致");
		}else{
			for (int i = 1; i <= page; i++){
				...
				if(i<page){
					Assert.assertEquals(String.valueOf(actual_size), String.valueOf(pagesize), "第"+i+"页获取待跟进需求数目与数据库不一致");
				}else{
					int end_size = except_size - pagesize *(i-1);
					System.out.println(end_size);
					Assert.assertEquals(String.valueOf(actual_size), String.valueOf(end_size), "第"+i+"页获取待跟进需求数目与数据库不一致");
				}
			}
